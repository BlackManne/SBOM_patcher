...
{
    "_id": {
      "$oid": "66e29397b70a6779390218b6"	//mongodb中的objectId
    },
    "No": "CVE-2024-8096",	//CVE编号
    "advisories_list": [	//该CVE对应的github advisories库中相关的url
    ...
    "affected_software": [	//该CVE影响的软件及对应的版本
      {
        //每一个json对象对应一个软件
        "software_name": "curl",	//软件名
        "interval_version": [		//离散的版本列表
          "-Up to(excluding)8.10.0-r0"
        ],
        "detail_version": [			//连续的版本列表
          "-Up to(excluding)8.10.0-r0"
        ],
        "raw_version": [		//原始的版本信息
          "-Up to(excluding)8.10.0-r0"
    ...
    "alicloud_list": [		//该CVE对应的aliyun漏洞库中相关的url
      "https://curl.se/docs/CVE-2024-8096.html",
      "https://curl.se/docs/CVE-2024-8096.json",
      "https://hackerone.com/reports/2669852"
    ],
    "cve_modified_time": null,		//该CVE的最新修改时间，若为初次入库则为null
    "cve_published_time": "2024-09-11",		//该CVE的发布时间
    "debian_list": [		//该CVE对应的debian漏洞库中相关的url
      "https://curl.se/docs/CVE-2024-8096.html",
    ...
    "exploit": [],		//该CVE对应的nvd漏洞库中标记为“揭露”（exploit）的url
    "github_advisories_patches": "",		//该CVE对应的github advisories库中patches字段
    "merge_time": "2024-09-12 15:09:11",	//入库时间
    "name": "使用 GnuTLS 绕过 OCSP 装订（CVE-2024-8096）",	//该CVE的漏洞名称
    "patch_list": {		
        //patch url列表及具体信息。
        //目前只有1.debian中被标记为fixed by等修复相关信息的url和2.nvd中被标记为patch的url
        //才会被归入此类
      "nvd": [],
      "debian": [
        { 
          "patch_url":  "https://github.com/curl/curl/commit/aeb1a281cab13c7ba791cb104e556b20e713941f",		//patch对应的url     
          "service_name": "github_commit",		//patch来源
          "patch_detail": {		//patch detail，根据patch来源不同具体的字段不同，以下是一个来自commit的示例
              					//patch基本都来自于github，因此格式取决于github api获取的字段格式
          ...
          "time": "2024-09-12 15:04:47"		//该patch入库时间
        }
      ]
    },
    "rate": "6.5",		//漏洞评级
    "score": "N/A",		//漏洞分数
    "source_urls": {	//该CVE编号对应的四个漏洞库的漏洞页面的url
    ...
    "third_party_list": [],		//该CVE对应的nvd漏洞库中标记为“第三方"（third_party）的url
    "title": "NVD - CVE-2024-8096",		//该CVE的标题
    "type": "未定义",		//该CVE的类型
    "vendor_list": []	//该CVE对应的nvd漏洞库中标记为“vendor"的url
  }
```
